...
通用表表达式（Comon Table Expression）能够实现查询结果的重复利用，因此可以用来<u>简化复杂的子查询和连接查询，提高SQL语句的可读性和性能，实现具有层次结构或者树状结构数据的遍历</u>。
...
SELECT * FROM t1[, t2];
```
其中，`WITH` 关键字表示定义通用表表达式（简称 CTE），因此通用表表达式也被称为 `WITH` 查询。t1 和 t2 指定了 CTE 的名称，后面**括号中的内容为<u>可选的字段名**</u>。`AS` 关键字后面的子查询是 CTE 的**定义语句**，定义<u>了它的表结构以及数据</u>。最后的 `SELECT` 是**主查询语句**，它可以引用前面定义的 CTE。除了 `SELECT`，主查询语句也可以是 `INSERT`、`UPDATE` 或 `DELETE` 等。 `WITH` 子句中可以定义多个 CTE，每个 CTE 之间使用逗号分隔，并且已经定义的 CTE 可以被后续的 CTE 引用。<br />举个简单的栗子：
```sql
...
```
## 递归查询
除提高查询的可读性和性能外，CTE 还**支持在定义中进行<u>自我引用**</u>，也就是实现了编程语言中的<u>递归调用</u>。递归形式的通用表表达式可以<u>用于遍历具有**层次结构**或者**树状结构**的数据</u>，例如遍历组织结构、查询地铁换路线图等。<br />其基本语法如下：
```sql
...
- cte_query_iterative：表示**递归查询语句**，可以对当前 CTE 进行自我引用。**每一次递归查询语句执行的结果都会再次作为输入，传递给下一次查询**。**如果递归查询无法从上一次迭代中返回更多的数据，将会终止递归**。最后，使用 `UNION [ALL]` 运算符合并这些结果集并进行输出。

> [!TIP|lable:递归 CTE 的执行顺序]
>
...
> [!NOTE]
> 递归查询必须要有一个终止递归的条件，否则的话，递归将会进入死循环！MySQL 默认递归1000次（由系统变量 cte_max_recursion_depth 控制）后终止递归，并提示错误。
>
> **递归条件**可以是**遍历完表中的所有数据后不再返回更多记录** 或者 **在递归查询部分的 WHERE 子句中指定一个终止条件**。
>
> 限制递归次数的终止条件必须写在 CTE 的定义中，写在主查询语句中是没有作用的。
...
DROP TABLE job;
```
员工表 employee 中存储了员工的各种信息，包括员工编号、姓名以及员工经理的编号。该公司的组织结构如下图所示，其中 "刘备" 没有上级，他的经理字段 manager 为空。<br />![](https://fastly.jsdelivr.net/gh/xihuanxiaorang/img/202310260035162.jpeg)<br />利用递归 CTE 生成一个组织结构图，显示每个员工从上到下的管理路径，SQL 语句如下所示：
```sql
...
对于好友这种关系，每个用户是一个顶点（Vertex），两个用户相互加为好友就会在两者之间建立一条边（Edge）。如下图所示，是一个简单的好友关系示意图。<br />![](https://fastly.jsdelivr.net/gh/xihuanxiaorang/img/202310260131226.jpeg)
...
而粉丝关系需要使用有向图（Directed Graph）表示。因为关注是单向关联，A 关注了B，但是 B 不一定关注 A。如下图所示，是一个简单的粉丝关系示意图。<br />![](https://fastly.jsdelivr.net/gh/xihuanxiaorang/img/202310260131965.jpeg)
...
FROM f1
         ...
         JOIN t_user u ON f1.friend_id = u.user_id;
```
...
```
#### 可能认识的人
社交软件通常实现了推荐好友的功能：一方面，其可能读取了该用户的手机通讯录，并找到已经在系统中注册，但不属于该用户好友的用户进行推荐；另一方面，其会<u>找出和该用户不是好友，但是有共同好友的用户，即好友的好友</u>，进行推荐。如下查找可以推荐给 "陈二" 的用户：
```sql
WITH friend(id) AS (SELECT friend_id
                    FROM t_friend
                    WHERE user_id = 2),
     fof AS (SELECT friend_id
             FROM t_friend f
...
WHERE friend_id not in (SELECT id FROM friend)
...
```
其中，relation 是一个递归 CTE。初始化查询语句用于查找 "赵六" 的好友，第一次递归返回了 "赵六" 好友的好友，然后依次类推。将关系层数限制为小于6，path 字段中存储了使用 "->" 分隔的关系链，INSTR 函数可以用于防止形成 A -> B -> A 这样的环路。查询结果如下所示：
```powershell
+------+------+------+------------------------+
| uid  | fid  | hops | path                   |
+------+------+------+------------------------+
...
+------+------+------+------------------------+
```
"赵六" 和 "孙七" 之间的关系是，通过 "李四" 和 "刘一" 两个人进行联系。
### 粉丝关系分析
![](https://fastly.jsdelivr.net/gh/xihuanxiaorang/img/202310260133715.jpeg)
...
WITH cf(user1, user2, followed) AS (
    SELECT d.user_id, r.follower_id, d.followed_id
    FROM t_followed d
...
FROM cf
JOIN t_user u1 ON user1 = u1.user_id
JOIN t_user u2 ON user2 = u2.user_id
JOIN t_user u3 ON followed = u3.user_id;
```
...
```
其中, df 代表了相关关注的两个用户。t_follower(粉丝表) 与 t_followed(关注表) 两表 JOIN 的连接条件如何获得？以用户 "刘一" 举例，

- 先获取 "刘一" 有哪些粉丝，然后再获取 "刘一" 所关注的人，推出两表连接的时候要是**同一个用户** "刘一"，所以 r.user_id = d.user_id；
- 因为一个表为粉丝表，一个表为关注表，都是针对同一个用户的，所以此时只需要再判断**该用户的粉丝是否存在于该用户所关注的人里面**，推出条件 r.follower_id = d.followed_id。
...
WITH RECURSIVE transfer(start_station, stop_station, stops, paths) AS (
    SELECT station_name, next_station, 1 AS stops,
           CAST(CONCAT(line_name, station_name, '->', line_name, next_station) AS CHAR(1000))
    FROM bj_subway WHERE station_name = '王府井'
                   UNION  ALL
                   SELECT t.start_station, s.next_station, t.stops + 1, CONCAT(t.paths, '->', s.line_name, s.next_station)
                   FROM transfer t
                   JOIN bj_subway s ON t.stop_station = s.station_name AND INSTR(t.paths, s.next_station) = 0
)
